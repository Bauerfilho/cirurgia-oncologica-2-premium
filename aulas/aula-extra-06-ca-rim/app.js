/* app.js — bootstrap da Aula Extra 6 (Câncer de Rim — Carcinoma de Células Renais)
   Reusa o chassi compartilhado: router · quiz universal · interativos (hotspots).
   Página única long-scroll com 11 seções (p1…p11), hash-router SPA.
   Header (brand · tema · home · hambúrguer direita) + menu overlay + page-nav + progress + PWA.
   As interações bespoke (tabela genética, card-âncora, hotspot venoso, step Stauffer,
   toggle sólido×cístico, escada Bozniak, fluxograma decisão) são wiradas por
   rim-interactive.js (script clássico com IIFE auto-init — fonte única de initAll).
   INIT ÚNICA: app.js NÃO chama o initAll do rim-interactive; o IIFE faz isso sozinho. */
...
const PAGE_TITLES = {
  p1: 'Quem desenvolve câncer de rim',
  p2: 'Duas síndromes que viram a questão',
  p3: 'A tríade rara que entrega tudo',
  p4: 'A varicocele que não devia estar ali',
  p5: 'Por que sempre o lado esquerdo',
  p6: 'A fosfatase que assustou a equipe',
  p7: 'Como o tumor aparece',
  p8: 'Sólido no rim: corta, não biopsia',
  p9: 'A escada de Bozniak',
  p10: 'Salvar o rim ou tirar inteiro',
  p11: 'Três casos, três vínculos',
};
const NAMESPACE = 'cironco2-aula-extra-06';
...
function initPageNav(router) {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // OU dentro de qualquer interação SVG (hotspot venoso, step, fluxograma, quiz):
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // rim-interactive.js (script clássico) roda seu próprio auto-init no carregamento
  // (IIFE no fim do arquivo). NÃO re-invocar aqui — fonte única de initAll é o IIFE
  // (evita registro duplo de listeners, inclusive focus dos hotspots).
  router.init();
});
