/* app.js — bootstrap da Aula Extra 7 (Câncer de Bexiga — Carcinoma Urotelial)
   Reusa o chassi compartilhado: router · quiz universal · interativos (hotspots).
   Página única long-scroll com 12 seções (p1…p12), hash-router SPA.
   Header (brand · tema · home · hambúrguer direita) + menu overlay + page-nav + progress + PWA.
   As interações bespoke (12 SVGs interativos) são wiradas por bexiga-interactive.js
   (script clássico com IIFE auto-init — fonte única de initAll).
   INIT ÚNICA: app.js NÃO chama o initAll do bexiga-interactive; o IIFE faz isso sozinho. */
...
const PAGE_TITLES = {
  p1: 'Por que a bexiga cai tanto em prova',
  p2: 'A bexiga é uma piscina de toxina',
  p3: 'O resto dos fatores de risco',
  p4: 'Urotélio: por que "transicional"',
  p5: 'Sangue sem dor: a pegadinha-mãe',
  p6: 'A bexiga irritada e a lesão-tapete',
  p7: 'A tríade que fecha o diagnóstico',
  p8: 'O divisor de águas: a camada muscular',
  p9: 'Não músculo-invasivo: o jogo do risco',
  p10: 'Músculo-invasivo: retirar a bexiga',
  p11: 'Conexões: HPB e CA de rim',
  p12: 'Fechamento: a pergunta que decide tudo',
};
const NAMESPACE = 'cironco2-aula-extra-07';
...
function initPageNav(router) {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // OU dentro de qualquer interação SVG (hotspot, step, slider, fluxograma, quiz):
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // bexiga-interactive.js (script clássico) roda seu próprio auto-init no carregamento
